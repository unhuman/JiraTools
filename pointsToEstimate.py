...
from libraries.jiraQueryTools import search_issues, build_points_estimate_query
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Copy story points to original estimate for tickets with story points but no original estimate.")
    ...
    try:
        jql_query = build_points_estimate_query(args.type, args.name)
    ...
    for issue in issues:
        ...
        if story_points is not None and original_estimate is None:
            estimate_value = convert_story_points_to_estimate(story_points)
            ...
            if args.perform_update:
                try:
                    ...
                    print(f"  ✓ Set remaining estimate to: {Fore.GREEN}0m{Style.RESET_ALL} (completed ticket)")
                    updated_count += 1
                except jira.exceptions.JIRAError as e:
                ...
                print(f"  Would set original estimate to: {Fore.GREEN}{estimate_value}{Style.RESET_ALL} (from {story_points} story points)")
                print(f"  Would set remaining estimate to: {Fore.GREEN}0m{Style.RESET_ALL} (completed ticket)")
                updated_count += 1
        else:
